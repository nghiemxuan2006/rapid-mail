...
export const importGmailSignatures = async (user: UserDocument) => {
    ...
    if (response.status >= 400) {
        ...
        throw new BAD_REQUEST_ERROR(errorData.error?.message || 'Failed to fetch signatures from Gmail API');
    }
...
export const importGmailSignaturesByAccount = async (userId: string, accountId: string) => {
    ...
    const tryFetch = (token: string) => sendRequest({
        method: 'GET',
        url: GMAIL_SEND_AS_ENDPOINT,
...
export const getDefaultSignatureForAccount = async (userId: string, accountId?: string) => {
    if (accountId) {
        ...
        if (account?.email) {
            const sig = await Signature.findOne({ userId, sourceEmail: account.email, isDefault: true }).lean();
            if (sig) return sig;
...
export const updateSignature = async (userId: string, signatureId: string, body: UpdateMySignatureBody) => {
    if (body.isDefault) {
        await Signature.updateMany({ userId }, { isDefault: false });
    }
